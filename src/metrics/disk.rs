...
use std::fs::{read_link, canonicalize};
use std::ffi::OsStr;
...
impl IoMetrics {
    pub fn from_io_counters(io_counters: &IoCounters) -> IoMetrics{
        IoMetrics { read_bytes: io_counters.read_bytes().get::<byte>(), 
...
impl ZDisk {
    pub fn from_disk(d: &Disk) -> ZDisk {
        ZDisk {
            ...
            previous_io: IoMetrics { read_bytes: 0, write_bytes: 0 },
            current_io: IoMetrics { read_bytes: 0, write_bytes: 0 },
        }
    }
    ...
    pub fn get_write_bytes_sec(&self, tick_rate: &Duration) -> f64 {
        (self.current_io.write_bytes - self.previous_io.write_bytes) as f64 / tick_rate.as_secs_f64()
    }
