...
fn render_process(
    ...
    if !app.gfx_devices.is_empty() {
        text.push(Span::raw("SM Util:            "));
        text.push(Span::styled(
            format!("{:7.2} %", p.sm_utilization as f64),
            rhs_style,
        ));
        text.push(Span::raw("\n"));
        text.push(Span::raw("Frame Buffer:       "));
        text.push(Span::styled(
            format!("{:7.2} %", p.fb_utilization as f64),
            rhs_style,
        ));
        text.push(Span::raw("\n"));
        text.push(Span::raw("Encoder Util:       "));
        text.push(Span::styled(
            format!("{:7.2} %", p.enc_utilization as f64),
            rhs_style,
        ));
        text.push(Span::raw("\n"));
        text.push(Span::raw("Decoder Util:       "));
        text.push(Span::styled(
    ...
    if text.len() > v_sections[1].height as usize * 3 {
        ...
        Paragraph::new(Spans::from(second_part))
            .block(Block::default())
            .wrap(Wrap { trim: false })
