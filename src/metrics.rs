...
fn load_sled_db(dur: Duration, path: &PathBuf, current_time: SystemTime, tick: Duration) -> Result<HistogramMap, Box<dyn Error>>{
    let db = sled::open(path)?;
    let mut map = HashMap::with_capacity(5);
    let mut log = fs::File::create(Path::new("log.txt")).expect("couldn't open log");
    ...
    Ok(HistogramMap{map, duration: dur, tick, db: Some(path.to_owned()), previous_stop: Some(previous_stop)})
}

fn load_and_migrate(dur: Duration, path: &PathBuf, current_time: SystemTime, tick: Duration) -> Option<HistogramMap>{
            ...
            Some(db) => {
                ...
                if sleddb.exists(){
                    match load_and_migrate(dur, db, current_time, tick){
                        Some(hm) => hm,
                        None =>  HistogramMap {map: HashMap::with_capacity(5), duration: dur, tick, db: path, previous_stop: None}
                    }
                }
                    ...
                    HistogramMap {map: HashMap::with_capacity(5), duration: dur,tick, db: path, previous_stop: None}
                }
            },
            None => HistogramMap {map: HashMap::with_capacity(5), duration: dur,tick, db: path, previous_stop: None}
        }
    ...
    fn save_histograms(&mut self) {
        match &self.db {
            Some(db) => {
                ...
                let dbfile = Path::new(db).join(Path::new("store"));
                let mut database = fs::OpenOptions::new().create(true).write(true).open(dbfile).expect("Couldn't Open DB");
                database.write(&bincode::serialize(self).expect(SER_ERROR)).expect("Failed to write file.");
                let configuration = Path::new(db).join(Path::new(".configuration"));
                let mut configuration = fs::OpenOptions::new().create(true).write(true).open(configuration).expect("Couldn't open Configuration");
                configuration.write(format!("version={:}\n", env!("CARGO_PKG_VERSION")).as_bytes()).expect("Failed to write file.");
            }
            None => {}
        }
    }
